...
bool compAng(int x, int y, int pivot, vector<Pto> &pts){
...
int main() {
  int H, E; 
  while(cin >> H >> E) {
    vector<Pto> pBuenos(H);
    vector<Pto> pMalos(E);
    forn(i,H) cin >> pBuenos[i].x >> pBuenos[i].y;
    forn(i,E) cin >> pMalos[i].x >> pMalos[i].y;

    // Ordenamos por coordenada Y y coordenada X, 
    sort(pBuenos.begin(), pBuenos.end());
    forn(i,H) pBuenos[i].id = i;
    ...
    forn(i,H){
      forn(j,H){
        forn(k,H){
          ...
          forn(z,E)
          if(puntoEnTriangulo(pMalos[z], pBuenos[i], pBuenos[j], pBuenos[k]))
          ...
          forn(z,H){
            ...
            if(puntoEnTriangulo(pBuenos[z], pBuenos[i], pBuenos[j], pBuenos[k]))
    ...
    forn(pivot,H-2){
      ...
      // Ordenamos los indices de los puntos en funcion del angulo con el pivot.
      sort(sigs.begin(),sigs.end(),bind(compAng,placeholders::_1,placeholders::_2,pivot,pBuenos));
      ...
      forn(i,ss){
        ...
        forsn(j,i+1,ss){
          ...
          forsn(k,j+1,ss){
            ...
            Pto pi = pBuenos[idxPi];
            Pto pj = pBuenos[idxPj];
            Pto pk = pBuenos[idxPk];
      ...
      forn(i,ss){
        ...
        forsn(j,i+1,ss){
          ...
          if(triangulosAdyacentes[k][i][j])
            if(dpTriangulos[k][i] + puntajePivotIJ + 1 > dpTriangulos[i][j])
              dpTriangulos[i][j] = dpTriangulos[k][i] + puntajePivotIJ + 1;
